...
describe Autoloader do
  ...
  before do
    ...
    # Remove class definitions and tell Ruby to allow files to be required again
    # before each test
  ...
  describe 'When a search path is added' do
    ...
    it 'will not print or puts' do
      proc {
        @autoloader.add_path("A")
        # Verifies that repeated path-adding behaves the same way
        @autoloader.add_path("A")
        begin
  ...
  describe "When a file is required" do
    it 'will indicate if the file is now or was already loaded' do
      @autoloader.add_path("A")
